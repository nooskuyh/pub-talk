...
// 0% = arm hanging down / extended (155 deg)
// 100% = bicep fully pulled / flexed (45 deg)
const bicepPull = ref(40)

const elbowAngle = computed(() => Math.round(155 - (bicepPull.value / 100) * 110))
...
const bicepMid = computed(() => {
  ...
  const bulge = 8 + 22 * (bicepPull.value / 100)
  return {
    x: mx + bulge,
    y: my,
    thickness: 8 + 14 * (bicepPull.value / 100),
  }
})
</script>
  ...
  <div class="minimal-wrapper">
    ...
    <div class="svg-box">
      <svg viewBox="0 0 340 220" class="minimal-svg">
        <defs>
          <marker id="arrow-pull" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
        ...
        <!-- Joint Angle Text -->
        <text :x="elbow.x - 20" :y="elbow.y + 28" class="angle-text" text-anchor="end">
          {{ elbowAngle }}°
        </text>
      </svg>
    </div>
    ...
    <div class="controls-box">
      <div class="slider-row">
        <span class="label-end">Relaxed (Straight)</span>
        <input
          v-model.number="bicepPull"
          type="range"
          min="0"
          max="100"
          class="minimal-slider coral-slider"
        >
      ...
      <div class="message-banner">
        <p class="main-msg">
          <strong>Muscles only pull—they cannot push.</strong>
        </p>
        <p class="sub-msg">
          The bicep can pull the arm up into a bend. When relaxed, it has no way to push the arm straight again.
        </p>
      </div>
    </div>
  </div>
...
.coral-slider::-webkit-slider-thumb {
  appearance: none;
  width: 18px;
  height: 18px;
  border-radius: 50%;
